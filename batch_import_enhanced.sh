...
LOG_FILE="import_enhanced_$(date +%Y%m%d_%H%M%S).log"
...
import_file "Data Element/Data Element.json" "Enhanced Data Elements (164 total)"

echo -e "\n${YELLOW}=== ENHANCED DASHBOARDS ===${NC}" | tee -a "$LOG_FILE"
import_file "Dashboard/Dashboard.json" "Cancer-Specific Dashboards (19 total)"
...
import_file "Data Set/Data Set.json" "Data Sets"

echo -e "\n${YELLOW}=== VALIDATION RULES ===${NC}" | tee -a "$LOG_FILE"
import_file "Validaion/Validation Rule Group.json" "Validation Rule Groups"
import_file "Validaion/Validation Rule.json" "Validation Rules"

echo -e "\n${YELLOW}=== VISUALIZATIONS ===${NC}" | tee -a "$LOG_FILE"
import_file "Visualisation/Visualisation.json" "Visualizations"
import_file "Event Visualisation/Even Visualisation.json" "Event Visualizations"

echo -e "\n${YELLOW}=== CANCER PROGRAMS (Re-import for completeness) ===${NC}" | tee -a "$LOG_FILE"
for program_file in Program/*Cancer\ Program.json; do
