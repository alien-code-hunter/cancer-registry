...
curl -s -X POST -H "Content-Type: application/json" -u 'Meduletu_Kamati:Covid19!#@$' 'http://localhost:8085/api/metadata?importStrategy=CREATE_AND_UPDATE&atomicMode=NONE' -d @"${BASE_DIR}/Data Element/Data Element.json" > /tmp/de_import.json
sleep 2

# 2. Validation Rules
echo "4. Importing Validation Rules (12)..."
curl -s -X POST -H "Content-Type: application/json" -u 'Meduletu_Kamati:Covid19!#@$' 'http://localhost:8085/api/metadata?importStrategy=CREATE_AND_UPDATE&atomicMode=NONE' -d @"${BASE_DIR}/Validation/Validation Rule.json" > /tmp/vr_import.json
sleep 2

# 3. Dashboards
echo "5. Importing Dashboards (19)..."
curl -s -X POST -H "Content-Type: application/json" -u 'Meduletu_Kamati:Covid19!#@$' 'http://localhost:8085/api/metadata?importStrategy=CREATE_AND_UPDATE&atomicMode=NONE' -d @"${BASE_DIR}/Dashboard/Dashboard.json" > /tmp/db_import.json
sleep 2

# 4. Datasets
echo "6. Importing Data Sets (unified)..."
curl -s -X POST -H "Content-Type: application/json" -u 'Meduletu_Kamati:Covid19!#@$' 'http://localhost:8085/api/metadata?importStrategy=CREATE_AND_UPDATE&atomicMode=NONE' -d @"${BASE_DIR}/Data Set/Data Set.json" > /tmp/ds_import.json
sleep 2
